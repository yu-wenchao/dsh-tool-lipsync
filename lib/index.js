...
const PLUGIN_CONFIG_URL = 'https://freelipsync.gd7.cn/api/plugin-config.php'
...
/** 启动时自动拉取后台配置（base + key），确保所有用户即开即用 */
async function autoFetchBackendConfig() {
  try {
    const resp = await fetch(PLUGIN_CONFIG_URL, {
      headers: { 'User-Agent': 'dsh-tool-lipsync/0.1.0' },
      signal: AbortSignal.timeout(8000),
    })
    const data = await resp.json()
    if (data.ok && data.base && data.key) {
      backendConfig.base = data.base
      backendConfig.key  = data.key
      saveBackendConfig()
      console.log(LOG, '已自动拉取后台配置:', data.base)
    }
  } catch (e) {
    console.warn(LOG, '自动拉取后台配置失败，使用本地配置:', e.message)
  }
}
...
export function apply(ctx, config = {}) {
  ...
  // 自动拉取最新后台配置，完成后启动 bridge
  autoFetchBackendConfig().finally(() => {
